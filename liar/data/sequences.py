...
# Code partially from advprompter
...
class Seq:
  def __init__(
    ...
    self._logits:typing.Optional[torch.Tensor] = None
    self._probs:typing.Optional[torch.Tensor] = None
    self._ids:typing.Optional[torch.Tensor] = None
    self._text:typing.Optional[list[str]|tuple[str]] = None
    self.mask:typing.Optional[torch.Tensor] = None

    self.eps = 1e-8

    if logits is not None: self.logits = logits
    elif probs is not None: self.probs = probs
    elif ids is not None: self.ids = ids
    elif text is not None: self.text = text
    elif tokens is not None: self.tokens = tokens
    else: raise RuntimeError("At least one field should be provided.")

    if mask is not None: self.mask = mask
    if self.mask is None: 
      self.mask = torch.ones((self.bs, self.seq_len), device=self.device).bool()
    if self.tokenizer.name_or_path != 'lvwerra/gpt2-imdb':
      pass

  @property
  def logits(self):
    if self.dtype == "logits":
      assert type(self._logits) is torch.Tensor
      logits = self._logits
    else: logits = torch.log(self.probs + self.eps)
  ...
  def probs(self):
    if self.dtype == "probs": 
      assert type(self._probs) is torch.Tensor
      probs = self._probs
    elif self.dtype == "logits": probs = torch.nn.functional.softmax(self.logits, dim=2)
    else: probs = self.onehot
    if torch.isnan(probs).any() or torch.isinf(probs).any():
      raise RuntimeError(f"NaN in retrieved probs: {probs}")
    return probs

  @probs.setter
  def probs(self, value:torch.Tensor):
    self._probs = value
  ...
  def ids(self):
    if self.dtype == "ids": 
      assert type(self._ids) is torch.Tensor
      ids = self._ids
    elif self.dtype == "logits": ids = torch.argmax(self.logits, dim=2)
    elif self.dtype == "probs": ids = torch.argmax(self.probs, dim=2)
    elif self.dtype == "text":
      ids:torch.Tensor = self.tokenizer.batch_encode_plus(
  ...
  @property
  def text(self):
    text:typing.Union[list[str],tuple[str]]
    if self.dtype == "text":
  ...
  def text(self, value):
    if not isinstance(value, list) and not isinstance(value, tuple):
      raise ValueError(f"Text should be a list or tuple of strings. Got {type(value)}.")
  ...
  @property
  def bs(self):
    if self.dtype == "probs": bs = self.probs.shape[0]
    elif self.dtype == "logits": bs = self.logits.shape[0]
    elif self.dtype == "ids": bs = self.ids.shape[0]
    elif self.dtype == "text":
      assert self._text is not None
      bs = len(self._text)
    return bs
  ...
  @property
  def seq_len(self):
    if self.dtype == "probs": seq_len = self.probs.shape[1]
    elif self.dtype == "logits": seq_len = self.logits.shape[1]
    else: seq_len = self.ids.shape[1]
    return seq_len
