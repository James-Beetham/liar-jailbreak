...
def compute_perplexity(id_seq, likelihood_seq, diff_max=False):
    logprobs = torch.gather(
        likelihood_seq.logprobs, dim=2, index=id_seq.ids.unsqueeze(2)
    ).squeeze(2)

    if diff_max:
        logprobs = likelihood_seq.logprobs.max(dim=2).values - logprobs
...
def log_data(
    ...
    bs = batch_size if batch_size is not None else prompter_ar.response_sample.bs # type: ignore
    log_dct = {}
    log_seqs = {
        "step": [step] * bs,
        "split": [split] * bs,
...
def llm_loader(llm_params:liar.conf.model.base_llm.LLMParams, verbose=False):
    tqdm.write(
    ...
    if llm_params.checkpoint == "stas/tiny-random-llama-2":
        ...
        model = AutoModelForCausalLM.from_pretrained(
            llm_params.checkpoint, dtype=dtype
        ).to(llm_params.device) # type: ignore
    elif llm_params.checkpoint == '':
        tokenizer = AutoTokenizer.from_pretrained(
            llm_params.checkpoint,
            padding_side="right",
        )
...
def get_embedding_matrix(model):
    if isinstance(model, GPTJForCausalLM) or isinstance(model, GPT2LMHeadModel):
        ...
        return model.base_model.embed_in.weight # type: ignore
    elif isinstance(model, FalconForCausalLM):
        return model.transformer.word_embeddings.weight # type: ignore
    else:
