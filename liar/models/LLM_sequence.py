...
def msg_to_seq(msg:str, tokenizer, device, context:dict[str,str]|None=None):
...
class MergedSeq:
    def __init__(self, seqs:list[Seq]):
    ...
    @property
    def eps(self): return self._seqs[0].eps
...
class Seq:
    ...
    def logits(self):
        ...
        if logits is None: raise ValueError(f'Logits was None.')
    ...
    def probs(self)->torch.Tensor:
        if self.dtype == "probs":
            ...
            probs = self.onehot
        if probs is None: raise ValueError(f'Probs was None.')
    ...
    def ids(self)->torch.Tensor:
        if self.dtype == "ids":
            ...
            #     print(f'Error in one of these lines:')
            #     print(f'\n'.join(self._text))
            #     pad_id = self.tokenizer.pad_token_id
            #     safe = self.tokenizer.encode(".", add_special_tokens=False)
            #     seqs = []
            #     for t in self._text:
            #         try:
            #             ids_ = self.tokenizer.encode(t if (t.strip()) else ".", add_special_tokens=False)
            #         except Exception as ex2:
            #             print(f'Error with string: {repr(t)}')
            #             print(ex2)
            #             ids_ = None
            #         seqs.append(torch.tensor(ids_ if ids_ else safe))
            #     ids = torch.nn.utils.rnn.pad_sequence(seqs, batch_first=True, padding_value=pad_id).to(self.device)
        else:
            raise RuntimeError("No data to retrieve ids from.")
        if ids is None: raise ValueError(f'Ids was None.')
        return ids 

    @property
    def ids_list(self)->list[torch.Tensor]:
        return [v for v in self.ids]
    ...
    def text(self)->list[str]:
        ...
        if text is None: raise ValueError(f'Text was None.')
        return text
    ...
    def text(self, value):
        ...
        #         if len(v.strip()) == 0: 
        #             self._text[i] = '.' # hack to avoid empty
        #             print(f'Got empty text.')
    ...
    def bs(self):
        if self.dtype == "probs":
            ...
            if self._text is None:
                raise ValueError(f'Text is None when trying to get bs.')
            bs = len(self._text)
        else: raise RuntimeError("No data to retrieve bs from.")
        return bs
            ...
            for t, m, e0 in zip(tok, mask, entr):
                ...
                if m == 0:
                    ...
                    if color_scheme == 1:
                        ...
                        color = (e, e / 2, 0)
                    else: raise ValueError(f'Invalid color_scheme: {color_scheme}')
    ...
    def __getitem__(self, _slice):
        ...
        if self.dtype == "logits":
            ...
            text = typing.cast(list[str], self._text)
            sliced_seq = Seq(
                text=[text[i] for i in original_slice],
                mask=self.mask[new_slice],
                tokenizer=self.tokenizer,
                device=self.device,
            )
    ...
    def repeat_interleave(self, times, dim=0):
        ...
        if self.dtype == "logits":
            ...
            assert dim == 0
            repeated_seq = Seq(
                text=[item for item in typing.cast(list[str], self._text) for i in range(times)],
                mask=self.mask.repeat_interleave(times, dim=dim),
                tokenizer=self.tokenizer,
                device=self.device,
            )
    ...
    def detach_(self):
        if self.dtype == "logits":
            ...
            typing.cast(torch.Tensor, self._ids).detach_()
        elif self.dtype == "text":
            pass
        else:
            raise RuntimeError("No data to detach.")
